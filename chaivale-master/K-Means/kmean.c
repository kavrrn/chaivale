...
void readPoints(FILE* ifp,Point *points, int num_points)
{
	...
	{
		fscanf(ifp,"%lf,%lf,%lf", &points[i]._r, &points[i]._m, &points[i]._n);
	}
}
...
{
	return sqrt((pow((point1._r-point2._r),2)));	
}
...
int main(int argc, char* argv[])
{
	...
	fprintf(ofp,"%d\n",y);
	for(i=0;i<K;i++)
	fprintf(ofp,"%d,%d,%d\n",(int)mean[i]._r,(int)mean[i]._m,(int)mean[i]._n);
	for(i=0;i<num_points;i++)
	fprintf(ofp,"%d,%d,%d,%d\n",(int)points[i]._r,(int)points[i]._m,(int)points[i]._n,after_cluster[i]+1);
	fclose(ofp);
